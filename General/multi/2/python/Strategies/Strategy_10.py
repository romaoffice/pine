...
#10 Uptrend Consolidation Long Scalp
# TF – 15m
# Indicators –
#     21 and 55 EMA 
#     PMARP default
#     PMARP 55EMA, 100 Lookback
# SL1 – Close below entry range wick low
# SL2 – Wick below prior low
# Entry 
#     21EMA > 55EMA
#     Entry after PMARP(55,100) first cross > 15
# Exit
# PMARP > 75
...
variation_list_Strategy_10=[
  {'pmarp_entry_level':15,'pmarp_exit_level':75},
  {'pmarp_entry_level':15,'pmarp_exit_level':73},
  {'pmarp_entry_level':17,'pmarp_exit_level':75},
  {'pmarp_entry_level':17,'pmarp_exit_level':73},
]


class Strategy_10(bt.Strategy):
    params = (
        ('percents_equity',1),
        ('pmarp_exit_level',75),
    ...
    def next(self, dt=None):
        ...
        dt = dt or self.datas[0].datetime.datetime(0)
        #condition
        self.firstCross=self.pmarp.pmarp[0]>self.p.pmarp_entry_level  and self.hitPmarpHit==True
        if(self.firstCross):
            self.hitPmarpHit==False
        if(self.pmarp.pmarp>75):
            self.hitPmarpHit==True

        long_entry_condition = self.fast_ema[0]>self.slow_ema[0] and self.firstCross
